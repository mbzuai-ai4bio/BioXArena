...
def grade(submission: pd.DataFrame, answers: pd.DataFrame) -> float:
    """Grade gene-disease association strength prediction using Pearson correlation."""
    validate_first_column_match(submission, answers)
    return grade_pearson(
        submission, answers, id_col="id", pred_col="disgenet_score", target_col="disgenet_score"
    )
