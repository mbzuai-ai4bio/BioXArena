...
def grade(submission: pd.DataFrame, answers: pd.DataFrame) -> float:
    ...
    # Case-insensitive exact match on answer choice (A/B/C/D)
    preds = submission["label"].astype(str).str.strip().str.upper()
    labels = answers["label"].astype(str).str.strip().str.upper()
