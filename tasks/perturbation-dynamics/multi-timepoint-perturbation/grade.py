...
def grade(submission: pd.DataFrame, answers: pd.DataFrame) -> float:
    """Compute mean Pearson correlation across all delta_expression dimensions
    for multi-timepoint perturbation predictions."""
    ...
    delta_cols = [c for c in answers.columns if c.startswith("delta_")]
    if not delta_cols:
