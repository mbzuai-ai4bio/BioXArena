import pandas as pd
from tasks.utils import grade_auprc
...
def grade(submission: pd.DataFrame, answers: pd.DataFrame) -> float:
    """Grade GRN inference using AUPRC over predicted regulatory edges."""
    validate_first_column_match(submission, answers)
    return grade_auprc(
        submission, answers, id_col="id", pred_col="label", label_col="label"
    )
