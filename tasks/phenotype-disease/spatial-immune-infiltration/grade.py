...
def grade(submission: pd.DataFrame, answers: pd.DataFrame) -> float:
    """Grade spatial immune infiltration prediction using average Pearson correlation
    across 6 immune marker genes."""
    validate_first_column_match(submission, answers)
    gene_cols = ["CD8A", "CD4", "FOXP3", "MS4A1", "CD68", "EPCAM"]
    submission = validate_submission(submission, answers, id_col="id", pred_cols=gene_cols)
    ...
    if not correlations:
        raise InvalidSubmissionError("No valid correlations could be computed")
